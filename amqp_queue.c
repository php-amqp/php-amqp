...
HashTable *amqp_queue_object_get_debug_info(zval *object, int *is_temp TSRMLS_DC) {
	...
	amqp_queue_object *queue = AMQP_QUEUE_OBJ_P(object);
	...
	*is_temp = 1;
	...
	/* Start adding values */
	ZVAL_STR(&value, zend_string_copy(queue->name));
	zend_hash_str_add(debug_info, "queue_name", sizeof("queue_name")-1, &value);

	if (ZSTR_LEN(queue->consumer_tag) > 0) {
		ZVAL_STR(&value, zend_string_copy(queue->consumer_tag));
	} else {
		ZVAL_NULL(&value);
	}
...
void amqp_queue_free_obj(zend_object *object TSRMLS_DC)
{
	...
	zend_string_release(queue->name);
	zend_string_release(queue->consumer_tag);
...
zend_object* amqp_queue_ctor(zend_class_entry *ce TSRMLS_DC)
{
	...
	queue->name = STR_EMPTY_ALLOC();
	queue->consumer_tag = STR_EMPTY_ALLOC();
...
PHP_METHOD(amqp_queue_class, getName)
{
	...
	if (ZSTR_LEN(queue->name)) {
		RETURN_STR(zend_string_copy(queue->name));
	} else {
		RETURN_FALSE;
	}
}
...
PHP_METHOD(amqp_queue_class, setName)
{
	...
	if (ZSTR_LEN(name) < 1 || ZSTR_LEN(name) > 255) {
	...
	/* Set the queue name */
	zend_string_release(queue->name);
	queue->name = zend_string_copy(name);
}
/* }}} */
...
PHP_METHOD(amqp_queue_class, getArgument)
{
	...
	if ((tmp = zend_hash_find(Z_ARRVAL(queue->arguments), key)) == NULL) {
		RETURN_FALSE;
	}
...
PHP_METHOD(amqp_queue_class, declareQueue)
{
	...
	amqp_queue_declare_ok_t *r = amqp_queue_declare(
		...
		channel->channel_id,
		php_amqp_zend_string(queue->name),
	...
	zend_string_release(queue->name);
	queue->name = php_amqp_bytes_t(r->queue);
...
PHP_METHOD(amqp_queue_class, bind)
{
	...
	zend_string *exchange_name;
	zend_string *keyname = NULL;

	amqp_table_t *arguments = NULL;

	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "S|Sa", &exchange_name, &keyname, &zvalArguments) == FAILURE) {
		return;
	}
	...
	amqp_queue_bind(
		...
		channel->channel_id,
		php_amqp_zend_string(queue->name),
		exchange_name ? php_amqp_zend_string(exchange_name) : amqp_empty_bytes,
...
PHP_METHOD(amqp_queue_class, consume)
{
	...
	zend_string *consumer_tag = NULL;

	zend_long flags = INI_INT("amqp.auto_ack") ? AMQP_AUTOACK : AMQP_NOPARAM;

	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "|f!lS", &fci,
									 &fci_cache, &flags, &consumer_tag) == FAILURE) {
		return;
	}
	...
	if (!(AMQP_JUST_CONSUME & flags)) {
		...
		amqp_basic_consume_ok_t *r = amqp_basic_consume(
				...
				channel->channel_id,
				php_amqp_zend_string(queue->name),
				(consumer_tag ? php_amqp_zend_string(consumer_tag) : amqp_empty_bytes), /* Consumer tag */
		...
		zend_string_release(queue->consumer_tag);
		queue->consumer_tag = php_amqp_bytes_t(r->consumer_tag);
	}
...
PHP_METHOD(amqp_queue_class, cancel)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "|S", &consumer_tag) == FAILURE) {
		return;
	}
    ...
    if (!consumer_tag && !ZSTR_LEN(queue->consumer_tag)) {
        return;
    }

	amqp_bytes_t consumer_tag_bytes = consumer_tag ? php_amqp_zend_string(consumer_tag) : php_amqp_zend_string(queue->consumer_tag);
	...
	if (!consumer_tag || zend_string_equals(consumer_tag, queue->consumer_tag) != 0) {
		zend_string_release(queue->consumer_tag);
		queue->consumer_tag = STR_EMPTY_ALLOC();
	}
...
PHP_METHOD(amqp_queue_class, getConsumerTag)
{
	...
	if (ZSTR_LEN(queue->consumer_tag) > 0) {
		RETURN_STR(zend_string_copy(queue->consumer_tag));
	}
