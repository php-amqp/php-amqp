...
static PHP_METHOD(amqp_queue_class, consume)
{
	...
	if (!(AMQP_JUST_CONSUME & flags)) {
		...
		if (PHP5to7_ZEND_HASH_FIND(HASH_OF(consumers), (const char *) r->consumer_tag.bytes, (PHP5to7_param_str_len_type_t) r->consumer_tag.len, consumer_tag_zv)) {
		...
		PHP5to7_ZEND_HASH_ADD(HASH_OF(consumers),
		                      (const char *) r->consumer_tag.bytes,
		                      (PHP5to7_param_str_len_type_t) r->consumer_tag.len,
	...
	while(1) {
		...
		if (!PHP5to7_ZEND_HASH_FIND(HASH_OF(consumers), (const char *) envelope.consumer_tag.bytes, (PHP5to7_param_str_len_type_t) envelope.consumer_tag.len, current_queue_zv)) {
			zend_throw_exception(amqp_exception_class_entry, "Orphaned consumer", 0 TSRMLS_CC);
			amqp_destroy_envelope(&envelope);
			break;
		}
...
static PHP_METHOD(amqp_queue_class, cancel)
{
	...
	if (PHP5to7_ZEND_HASH_FIND(HASH_OF(consumers), (const char *) r->consumer_tag.bytes, (PHP5to7_param_str_len_type_t) r->consumer_tag.len, tmp)) {
		PHP5to7_ZEND_HASH_DEL(HASH_OF(consumers), (const char *) r->consumer_tag.bytes, (PHP5to7_param_str_len_type_t) r->consumer_tag.len);
	}

	add_assoc_null_ex(consumers, (const char *) r->consumer_tag.bytes, (PHP5to7_param_str_len_type_t) r->consumer_tag.len);
