...
static PHP_METHOD(amqp_queue_class, consume)
{
	...
	zval *consumers = zend_read_property(amqp_channel_class_entry, PHP5to8_OBJ_PROP(channel_zv), ZEND_STRL("consumers"), 0 , &rv TSRMLS_CC);
	...
	if (!(AMQP_JUST_CONSUME & flags)) {
		...
		zend_update_property_stringl(this_ce, PHP5to8_OBJ_PROP(getThis()), ZEND_STRL("consumer_tag"), (const char *) r->consumer_tag.bytes, (size_t) r->consumer_tag.len TSRMLS_CC);
	}
	...
	while(1) {
		...
		consumers = zend_read_property(amqp_channel_class_entry, PHP5to8_OBJ_PROP(&current_channel_zv), ZEND_STRL("consumers"), 0 , &rv TSRMLS_CC);
		...
		if ((current_queue_zv = zend_hash_str_find(Z_ARRVAL_P(consumers), key, envelope.consumer_tag.len)) == NULL) {
		    ...
		    zend_update_property_string(zend_exception_get_default(TSRMLS_C), PHP5to8_OBJ_PROP(&exception),  ZEND_STRL("message"), "Orphaned envelope" TSRMLS_CC);
			zend_update_property(amqp_envelope_exception_class_entry, PHP5to8_OBJ_PROP(&exception), ZEND_STRL("envelope"), &message TSRMLS_CC);
